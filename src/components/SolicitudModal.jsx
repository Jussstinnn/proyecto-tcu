...
export default function SolicitudModal({ isOpen, onClose, solicitudData, onApprove, onReject, onReturn }) {
      ...
      <div className="relative w-full max-w-2xl p-6 bg-white rounded-lg shadow-xl">
        ...
        <div className="mt-6 space-y-4 max-h-[60vh] overflow-y-auto pr-2">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div><strong>Carrera:</strong> {formData.carrera}</div>
          </div>
          ...
          <div className="space-y-2 text-sm">
            ...
            <div>
              ...
              <p className="p-2 bg-gray-50 rounded-md border">{formData.justificacion}</p>
            </div>
            <div>
              <strong>Objetivo General:</strong>
              <p className="p-2 bg-gray-50 rounded-md border">{formData.objetivoGeneral}</p>
            </div>
            <div>
          ...
          {}
          <h4 className="text-lg font-semibold mt-4">Observaciones del Revisor</h4>
          <textarea
            value={observation}
            ...
            readOnly={status !== 'Enviado' && status !== 'Observado' && status !== 'En Revisión' && status !== 'Open'}
          ></textarea>
        ...
        <div className="flex justify-end pt-6 mt-6 space-x-3 border-t">
          {}
          {(status === 'Enviado' || status === 'Observado' || status === 'En Revisión' || status === 'Open') ? (
            <>
              <button
