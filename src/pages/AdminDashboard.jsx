...
import { LuLayoutDashboard, LuMail, LuCalendar, LuTicket, LuFile, LuUsers, LuSettings } from "react-icons/lu";
        ...
        <aside className="w-64 bg-gray-900 text-gray-300 p-6 flex flex-col">
          ...
          <nav className="flex-1">
            <ul className="space-y-3">
              <li>
                <a href="/admin" className="flex items-center space-x-3 p-2 rounded-md bg-blue-600 text-white">
              ...
              <li>
                <a href="/instituciones" className="flex items-center space-x-3 p-2 rounded-md hover:bg-gray-700">
        ...
        <main className="flex-1 p-8 overflow-y-auto">
          <div>
            ...
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-gray-800">Bandeja de Solicitudes</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <SummaryCard title="Total Solicitudes" value={solicitudes.length} color="blue" />
              <SummaryCard title="Pendientes" value={solicitudes.filter(s => s.status === 'Enviado' || s.status === 'Open').length} color="yellow" />
              <SummaryCard title="Aprobadas" value={solicitudes.filter(s => s.status === 'Aprobado' || s.status === 'Closed').length} color="green" />
              <SummaryCard title="Rechazadas" value={solicitudes.filter(s => s.status === 'Rechazado').length} color="red" />
            </div>
              ...
              <table className="w-full">
                ...
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="p-4 text-left text-sm font-semibold text-gray-600">Estudiante</th>
                    <th className="p-4 text-left text-sm font-semibold text-gray-600">Asunto</th>
                    ...
                    <th className="p-4 text-left text-sm font-semibold text-gray-600">Vencimiento</th>
                ...
                <tbody>
                  {solicitudes.map((ticket) => (
                    <tr key={ticket.id} className="border-b border-gray-200 hover:bg-gray-50">
                      {}
                      <td className="p-4 text-sm text-gray-800 font-medium">{ticket.id}</td>
                      <td className="p-4 text-sm text-gray-700">{ticket.req}</td>
                      <td className="p-4 text-sm text-gray-700">{ticket.subj}</td>
                      <td className="p-4 text-sm">
                        <span className={`px-3 py-1 rounded-full font-medium text-xs ${getPriorityClass(ticket.prio)}`}>
                          {ticket.prio}
                        </span>
                      </td>
                      <td className="p-4 text-sm">
                        <span className={`px-3 py-1 rounded-full font-medium text-xs ${getStatusClass(ticket.status)}`}>
                          {ticket.status}
                        </span>
                      </td>
                      <td className="p-4 text-sm text-gray-700">{ticket.due}</td>
