...
function normalizeStatusLabel(status) {
  ...
  if (s === "En revisi贸n") return "En Revisi贸n";
...
async function createSolicitud(req, res) {
  ...
  if (!cronogramaValidation.isValid) {
    return res.status(400).json({
      message: `El cronograma debe tener filas completas y sumar m韓imo ${REQUIRED_TCU_HOURS} horas.`,
    ...
    await pool.query(
      `UPDATE solicitudes
       SET assigned_to = ?,
           assign_date = NOW(),
           estado = CASE
             WHEN estado IN ('Enviado', 'Observado') THEN 'En Revisi贸n'
             ELSE estado
           END,
           updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
...
async function returnSolicitudWithFlags(req, res) {
  ...
  if (!observation || !String(observation).trim()) {
    return res.status(400).json({
      message: "La observaci贸n es requerida para devolver la solicitud.",
    });
  }
  ...
  if (
    ...
    return res.status(400).json({
      message: "Debes habilitar al menos una secci贸n para correcci贸n.",
    });
  }
    ...
    await conn.query(
      ...
      [
        ...
        `Secciones habilitadas: ${
          [
            flags.institucion_editable ? "Instituci贸n" : null,
            ...
            .filter(Boolean)
            .join(", ") || "Ninguna"
        }. Observaci贸n: ${String(observation).trim()}`,
      ],
    );
...
async function resubmitSolicitud(req, res) {
  ...
  try {
    ...
    if (updateFields.length) {
      updateFields.push("estado = 'En Revisi贸n'");
    ...
    await conn.query(
      ...
      [
        ...
        actorEmail,
        "El estudiante actualiz贸 las secciones observadas y reenvi贸 el anteproyecto.",
      ],
    );
  ...
  try {
    ...
    const resumen = {
      ...
      pendientes: normalizedRows.filter((r) =>
        ["Enviado", "En Revisi贸n"].includes(r.estado_normalizado),
      ).length,
    };
    ...
    for (const row of normalizedRows) {
      const institucion = row.institucion_nombre || "Sin instituci贸n";
      ...
      if (["Enviado", "En Revisi贸n"].includes(estado)) {
