...
async function createSolicitud(req, res) {
  const {
    ...
    // institución (solo lo que vive en solicitudes)
    institucion_id,
    institucion_nombre,

    // anteproyecto
    titulo_proyecto,
    descripcion_problema, // ✅ nombre real en BD
    justificacion, // compat

    objetivo_general,
    objetivos_especificos, // legacy (texto opcional)
    beneficiario,
    estrategia_solucion,
  ...
  try {
    ...
    // 1) Insert principal (solicitudes)
    const [result] = await conn.query(
      ...
      [
        ...
        descProblema,
        objetivo_general,
        objetivos_especificos || null, // legacy opcional
        beneficiario || null,
    ...
    // 3) Cronograma (tabla hija) horas INT
 ...
 * Devuelve: solicitud + objetivos + cronograma + history
 */
